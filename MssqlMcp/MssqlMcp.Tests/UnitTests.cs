...
namespace MssqlMcp.Tests
{
    public sealed class MssqlMcpTests : IDisposable
    {
        ...
        [Fact]
        public async Task ReadData_ReturnsError_WhenDatabaseIsUnknown()
        {
            var result = await _tools.ReadData("SELECT 1", "no_such_database") as DbOperationResult;
            Assert.NotNull(result);
            Assert.False(result.Success);
            Assert.Contains("Unknown database 'no_such_database'", result.Error ?? string.Empty, StringComparison.Ordinal);
        }

        [Fact]
        public async Task ListTables_ReturnsError_WhenDatabaseIsUnknown()
        {
            var result = await _tools.ListTables("no_such_database") as DbOperationResult;
            Assert.NotNull(result);
            Assert.False(result.Success);
            Assert.Contains("Unknown database 'no_such_database'", result.Error ?? string.Empty, StringComparison.Ordinal);
        }
        ...
        [Fact]
        public void ListDatabases_ReturnsNamesWithoutConnectionStrings()
        {
            var result = _tools.ListDatabases();
            Assert.True(result.Success);

            // The single configured database is CONNECTION_STRING, which the factory registers
            // as "default". Asserting the exact names catches a regression that returned the
            // connection strings themselves.
            var names = Assert.IsAssignableFrom<IReadOnlyList<string>>(result.Data);
            Assert.Equal(new[] { "default" }, names);
        }
    }
}
